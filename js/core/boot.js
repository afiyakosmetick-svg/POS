...
import { seedDemo } from '../data/seed.js';
...
/**
 * Non-destructive: add any permissions a system role preset gained since this
 * browser last seeded, without touching custom roles or removing anything.
 */
async function syncSystemRolePermissions() {
  try {
    ...
    for (const preset of ROLE_PRESETS) {
      ...
      const row = roles.get(preset.id) || roles.all().find((r) => r.system && r.name === preset.name);
      if (!row) continue;
      const merged = [...new Set([...(row.permissions || []), ...preset.permissions])];
      if (merged.length !== (row.permissions || []).length) {
        roles.update(row.id, { permissions: merged });
        changed++;
      }
    }
    if (changed) console.info(`[boot] synced ${changed} system role permission set(s)`);
  } catch (err) {
    console.warn('[boot] role permission sync skipped', err);
  }
}
