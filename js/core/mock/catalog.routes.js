...
function normalizeProduct(body, existing = null) {
  ...
  const variants = (body.variants || []).map((v) => ({
    ...
    barcode: v.barcode || generateEan13(Date.now() + Math.random() * 1000),
  ...
  return {
    ...
    barcode: body.barcode || existing?.barcode || generateEan13(Date.now()),
...
export default function register(router) {
  ...
  router.post('/products/:id/duplicate', ({ params }) => {
    ...
    return db.tx(() => {
      const copy = normalizeProduct({
        ...
        barcode: generateEan13(Date.now()),
        variants: (src.variants || []).map((v) => ({ ...v, id: uuid(), sku: v.sku + '-C', barcode: generateEan13(Date.now() + Math.random() * 999), openingStock: 0 })),
      });
  ...
  router.post('/barcode/generate', ({ body }) => {
    ...
    const codes = Array.from({ length: count }, (_, i) => generateEan13(Date.now() + i * 37));
    return ok({ codes });
  });
