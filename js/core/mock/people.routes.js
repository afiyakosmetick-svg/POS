...
export default function register(router) {
  ...
  router.get('/employees', ({ query }) => {
    const rows = db.collection('users').all().filter((u) => query.includeArchived === 'true' || !u.archivedAt).map((u) => {
  ...
  router.get('/employees/:id', ({ params }) => {
    const u = db.collection('users').get(params.id);
  ...
  router.patch('/employees/:id', async ({ params, body }) => {
    const user = db.collection('users').get(params.id);
    ...
    if ((body.status === 'inactive' || body.roleId) && isLastOwner(user)) {
      conflict('You are the only Business Owner. Assign another owner before changing this account.');
    }
  ...
  router.del('/employees/:id', ({ params }) => {
    ...
    if (isLastOwner(user)) conflict('Cannot deactivate the only Business Owner.');
    return db.tx(() => {
...
function isLastOwner(user) {
  ...
  if (!ownerRole || user.roleId !== ownerRole.id) return false;
  return db.collection('users').count((u) => u.roleId === ownerRole.id && u.status === 'active') <= 1;
}
