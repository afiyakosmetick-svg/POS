 ...
 * opts: { cartDiscountType, cartDiscountValue, taxes: [{id,rate,inclusive}], roundTo }
 */
export function computeCart(lines, opts = {}) {
  ...
  // Pass 2: cart-level discount distributed across lines by net weight
  const netSum = rounded.reduce((s, l) => s + l.netAfterLineDiscount, 0);
  let cartDiscount = 0;
  if (opts.cartDiscountType === 'percent') cartDiscount = money.percent(netSum, opts.cartDiscountValue || 0);
  else if (opts.cartDiscountType === 'fixed') cartDiscount = money.toMinor(opts.cartDiscountValue || 0);
  cartDiscount = Math.min(Math.max(0, cartDiscount), netSum);
  ...
  const grand = items.reduce((s, l) => s + l.lineTotal, 0);
