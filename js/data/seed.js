/**
 * seed.js - realistic demo data for "Afia Cosmetics".
 *
...
export async function seedBlank(db) {
  ...
  const ownerRole = db.collection('roles').all().find((r) => r.name === 'Business Owner');
...
export async function seedDemo(db) {
  ...
  /* business + branches */
  const business = db.collection('businesses').insert({
    id: uuid(), name: 'Afia Cosmetics', legalName: 'Afia Cosmetics & Beauty Ltd.',
    logoId: null, address: 'House 24, Road 11, Banani, Dhaka 1213',
    phone: '+880 1711 004488', email: 'hello@afiacosmetics.shop', website: 'afiacosmetics.shop',
    vatNo: 'BIN-000123456-0202', currency: 'BDT', currencySymbol: '৳',
  });
  const branchMain = db.collection('branches').insert({ id: uuid(), name: 'Banani Flagship', code: 'BAN', address: business.address, phone: business.phone, email: 'banani@afiacosmetics.shop', isDefault: true, status: 'active' });
  const branchGulshan = db.collection('branches').insert({ id: uuid(), name: 'Gulshan Branch', code: 'GUL', address: 'Pink City, Gulshan 2, Dhaka', phone: '+880 1711 004499', email: 'gulshan@afiacosmetics.shop', isDefault: false, status: 'active' });
  ...
  const staff = [
    { name: 'Afia Rahman', email: 'admin@afiacosmetics.shop', role: 'Business Owner', branches: [branchMain.id, branchGulshan.id], phone: '+8801711004488' },
    { name: 'Tania Islam', email: 'manager@afiacosmetics.shop', role: 'Manager', branches: [branchMain.id], phone: '+8801811223344' },
    { name: 'Rakib Hasan', email: 'cashier@afiacosmetics.shop', role: 'Cashier', branches: [branchMain.id], phone: '+8801911556677' },
    { name: 'Nusrat Jahan', email: 'cashier2@afiacosmetics.shop', role: 'Cashier', branches: [branchGulshan.id], phone: '+8801611889900' },
    { name: 'Sabbir Ahmed', email: 'inventory@afiacosmetics.shop', role: 'Inventory Manager', branches: [branchMain.id, branchGulshan.id], phone: '+8801511332211' },
    { name: 'Farhana Akter', email: 'accounts@afiacosmetics.shop', role: 'Accountant', branches: [branchMain.id], phone: '+8801411778866' },
  ];
  ...
  const owner = users[0];
  const cashierMain = users[2];
  const cashierGul = users[3];
  ...
  db.collection('notifications').insert({ id: uuid(), type: 'system', title: 'Welcome to Afia Cosmetics POS', message: 'Demo data is loaded. Reset any time from Settings → Backup / Data Management.', level: 'info', read: false, link: '#/help', meta: {}, at: daysAgo(0) });
...
function defaultSettings(business) {
  return {
    ...
    business: {
      ...
      currency: 'BDT', currencySymbol: '৳', invoicePrefix: 'AFIA',
    },
    pos: {
      invoiceTemplate: 'AFIA-{BR}-{SEQ}', receiptSize: '80', printAfterSale: true,
    ...
    receipt: {
      header: 'Afia Cosmetics', footer: 'Thank you for shopping with us!\nExchange within 7 days with receipt.',
