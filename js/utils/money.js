...
/** Display format: 1234567 -> "৳ 12,345.67" */
export function format(minor, { withSymbol = true, symbol = config.locale.currencySymbol } = {}) {
  const neg = (minor || 0) < 0;
  const abs = Math.abs(Math.trunc(minor || 0));
  const major = Math.trunc(abs / FACTOR);
  const frac = String(abs % FACTOR).padStart(MINOR, '0');
  const grouped = String(major).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
  const body = MINOR > 0 ? `${grouped}.${frac}` : grouped;
