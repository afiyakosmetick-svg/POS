...
export function buildReceipt(sale, { settings = {} } = {}) {
  ...
  const header = `<div class="rc-header">
    ${cfg.showBusinessName ? `<div class="rc-biz">${escapeHtml(cfg.headerText || biz.name || 'Afia Cosmetics')}</div>` : ''}
  ...
  /* ---- totals ---- */
  const taxRows = (sale.taxLines || []).map((t) => row(`${t.name} (${t.rate}%)`, n(t.amount))).join('');
...
function style(cfg, sz) {
  ...
  return `<style>
    ...
    @media print {
      .receipt-preview.inv-doc { box-shadow: none !important; margin: 0 !important; ${auto ? 'min-height:auto;' : ''} }
    }
  </style>`;
}
