...
export default async function dashboardPage(ctx, mount) {
  ...
  const preset = custom ? 'custom' : q.preset || 'this_month';
  ...
  const cards = [
    ...
    renderKpi({ label: 'Total Discount', value: m(k.totalDiscount), icon: 'percent', tone: 'warning', href: reportHref('discounts') }),
