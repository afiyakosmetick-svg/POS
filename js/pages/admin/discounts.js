...
export default async function discountsPage(ctx, mount) {
  resourcePage(mount, {
    title: 'Discounts',
    subtitle: 'Cart, product and category discounts plus coupon codes for the POS.',
    ...
    columns: [
      { key: 'name', label: 'Name', sortable: true, render: (r) => `<strong>${escapeHtml(r.name)}</strong>${r.code ? ` ${pill(r.code, 'brand')}` : ''}` },
      { key: 'type', label: 'Value', render: (r) => r.type === 'percent' ? `${r.value}%` : money.format(money.toMinor(r.value)) },
    ...
    formFields: () => [
      ...
      { name: 'type', label: 'Type', type: 'select', required: true, options: [{ value: 'percent', label: 'Percentage' }, { value: 'fixed', label: 'Fixed amount' }] },
