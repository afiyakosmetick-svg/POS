...
export default async function reportsPage(ctx, mount) {
  ...
  let preset = custom ? 'custom' : q.preset || 'this_month';
  const filters = { ...q };
  ...
  const shell = pageShell(mount, {
    ...
    subtitle: `${store.get('business')?.name || 'Afia Cosmetics'}${branch ? ' · ' + branch.name : ''}`,
  ...
  function doPrint() {
    ...
    printHtml(`<div class="receipt-preview size-a4" style="padding:12mm">
      ...
      <p style="color:#666;font-size:11px;margin-bottom:6mm">${escapeHtml(store.get('business')?.name || 'Afia Cosmetics')} · ${escapeHtml(rangeLabel())} · generated ${new Date().toLocaleString()}</p>
      ${table}
    </div>`);
  }
