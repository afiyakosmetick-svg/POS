...
export async function renderPOS(mount, { onNeedRegister } = {}) {
  ...
  /* -------- data load -------- */
  try {
    const [taxRes, catTree, s] = await Promise.all([
  ...
  mount.querySelector('.js-scan').addEventListener('click', () => cameraScan((code) => {
    els.barcode.value = code;
    els.barcode.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter' }));
  }));
  ...
  function renderSummary({ totals }) {
    ...
    els.summary.innerHTML = `
      ...
      ${t.discountTotal ? `<div class="summary-row"><span>Discount</span><span class="pos-amount text-danger">−${money.format(t.discountTotal)}</span></div>` : ''}
      ${t.taxLines.map((tl) => `<div class="summary-row"><span>${escapeHtml(tl.name)} (${tl.rate}%)</span><span class="pos-amount">${money.format(tl.amount)}</span></div>`).join('')}
    ...
    const m = openModal({ title: 'Cart Discount', size: 'sm', body: `
      <div class="stack" style="--stack-gap:var(--sp-3)">
        <div class="segmented" role="group">
          <button data-t="percent" aria-pressed="${cd.type === 'percent'}">Percentage</button>
          <button data-t="fixed" aria-pressed="${cd.type === 'fixed'}">Fixed amount</button>
        </div>
  ...
  function editLineDiscount(lineId) {
    ...
    const m = openModal({ title: `Discount — ${line.name}`, size: 'sm', body: `
      <div class="stack" style="--stack-gap:var(--sp-3)">
        <div class="segmented"><button data-t="percent" aria-pressed="${line.discountType === 'percent'}">%</button><button data-t="fixed" aria-pressed="${line.discountType === 'fixed'}">Fixed / unit</button></div>
