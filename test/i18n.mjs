...
const dom = new JSDOM('<!doctype html><html><head><title>Dashboard · Afia Cosmetics</title></head><body><div id="app-root"></div></body></html>', { url: 'http://localhost:5173/admin.html', pretendToBeVisual: true });
const { window } = dom;
...
T('<title> translated to Bangla', document.querySelector('title').textContent === 'ড্যাশবোর্ড · Afia Cosmetics', document.querySelector('title').textContent);

// switch back to English restores originals
applyLang('en');
T('<title> restored to English', document.querySelector('title').textContent === 'Dashboard · Afia Cosmetics', document.querySelector('title').textContent);
